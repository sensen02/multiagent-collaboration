...
- 左侧任务列表：搜索、状态筛选、待回答问题数量；窄屏折叠为抽屉。底部是**「已归档会话」**——结案的运行从这里进，默认折叠、按归档时间倒序，点开仍可查看全部记录。
...
- **对话栏**（对话页底部，常驻）：像正常对话一样给 Agent 补一句话，不必再走「修改目标」（后者是切换目标版本，会停止旧任务树）。它复用既有消息协议 `POST /api/message` → `deliver(delivery='wake')`，因此语义是确定的：消息先进收件箱，**当前这一轮（模型回复 + 它请求的工具批次）结束后被读到**；正在等待的任务立刻被唤醒；已交付但仍属当前版本的任务会被重新排队，在同一条对话里继续（不新建版本、不丢历史）；运行处于暂停时，发送会先恢复运行再投递。发给**当前选中的那个 Agent**；若它已被目标切换弃置（旧版本或 superseded），输入栏会改发给当前版本的主调度并说明原因——否则消息投进去没有任何东西会被唤醒。草稿、阅读位置与焦点都由前端自己留住（SSE 每有新事件就重画整个主区）：重画前以 **DOM** 为准取一次快照（草稿可能比最后一次 `input` 事件更新），重画后写回文本、**重新聚焦并恢复光标**——旧实现只在"取样那一刻 `activeElement` 正好是输入框"时才重新聚焦，否则之后的字全进了空气。输入法**组词期间不重画**（重画会把候选与没上屏的字一起丢掉），组词结束后补一次刷新。任务变成终态、运行被停止时，输入框也不再整块消失：草稿留在屏幕上（不可编辑），等有任务可接收时接着发。
- 顶栏：连接状态、“技能”与“模型”两个入口；控制台只跟随系统深浅色，不提供手动主题切换。
- **长目标会自己让位**：**两处**目标都管——页面顶部的**主目标**（`.run-title`）与当前 Agent 的目标（`.chat-goal`）。实测真机的主目标正文 3465 字渲染成 **2912px**（窄屏 13216px），挂在所有标签页上方，一个人比整个可视区还高，往下滚几百像素看到的还是它，对话起点在 y=4799（窗口 960）。现在**超过 3 行**的目标在下滑时自动折叠成 2 行（带「展开目标」按钮），**向上滚到头自动展开**；短目标完全不进这个机制，也不会多出按钮。实现上两件事必须说清：触发看"目标被推出视野多少"而不是某个固定容器的 `scrollTop`（真正的滚动容器是 `main.main`，长目标下 `.chat-scroll` 只有 115px、根本滚不动），展开听的是**向上滚的手势**而不是滚动事件（折叠会让页面变短、浏览器把 `scrollTop` 夹回 0，此后没有可滚空间，滚动事件永远不来）。折叠按被收起的高度补偿滚动位置（有空间才补偿），正在看的内容不会跳。
...
- 读写/精确编辑/执行 shell、文件差异、内容寻址版本、归档结案（停进程 + 释放工作区副本 + 保留文件修改记录）及恢复到新目录。
...
归档是**结案**，不是"再存一份"，见下面「归档：把会话收起来」一节。恢复始终创建新目录，不覆盖当前项目。核心需求改变不会自动回滚已经写入根目录的内容；新根 Agent 获得变更说明后决定如何调整。
...
**归档是结案，不是"再存一份"。** 它回答的是"这个运行已经结束了，我要的是不再占着位置、也不再占着磁盘，但记录一条都不能少"，所以它做五件事，顺序是有意的：

| 步骤 | 做什么 | 为什么是这个顺序 |
|---|---|---|
| **门禁** | 只有 **已完成 / 已停止 / 主调度模型报错**（根任务 `failed`，不会再有新调度）时才允许 | 归档会停掉这个运行的一切，还在跑的时候做等于把工作扔了。进行中会明确拒绝并说明该先做什么 |
| **停掉一切** | 该 run 的所有非终态任务一律取消；`cancel_task` 会 `SIGKILL` 任务派生的**整个进程组**，终态任务残留的进程也一并收掉 | 留一个 30 秒的 shell 挂在那里，就不算结案 |
| **落冷备** | `workspace.archive()` 先把事件、报告、知识、文件版本与内容对象写进 `<数据目录>/archives/*.tar.gz` | **先有退路再释放**：`restore_archive` 随时能解回一份完整工作区到新目录 |
| **释放副本** | 删除 `<数据目录>/workspaces/<task_id>`（`setup()` 给每个子任务建的副本，运行结束后再没人用它） | 磁盘真正回收的地方。只删数据目录工作区根下面、且目录名等于任务 id 的目录；根任务直接操作用户项目目录，永远不在范围内 |
| **保留文件修改记录** | `file` 记录、内容对象、报告、事件**一律不动** | 释放副本会让"目录不存在"看起来像"文件全被删了"，而那批删除从未发生过——所以 `release()` 与删除同一笔写下 `workspace_released`，此后该任务的任何一次对账都不再把缺失当删除 |

归档后：

- run 记录打上 `archived` 与 `archive_path`，写一条 `RunArchived` 事件（含被停任务、被释放副本、释放字节数、保留的文件记录条数）；
- 控制台把它从活动列表移到左侧底部的 **「已归档会话」**——默认折叠、按归档时间倒序、**一条记录都不删**，点开照旧能看协作图、轨迹、文件版本与事件；
- **这个运行不再继续**：归档后 `revise` / `resume` / `resume_run` / 新建子任务 / `wake` 唤醒一律拒绝并说明原因。释放副本之后接着跑是危险的：工作区已经不在磁盘上了；
- `workspace-scan` 也不再对它做两秒一轮的文件对账（记录已冻结）。

一句话：**归档把会话从"活的"变成"可查的"**，而不是把记录变小或删掉。
...
```bash
python3 tests/run_all.py                    # 全部套件（当前 326 个用例）
...
所有 HTTP 请求都由运行时的**同一个事件循环**串行处理，所以"每个接口都不慢"不等于"界面不卡"——一个慢请求会把整页拖住。2026-09-15 的实测把这条链拆开过（详见 [实施记录](实施记录.md) 第十轮）：

- 服务启动不是瓶颈（`systemctl restart` 到能响应 HTTP 实测 **0.27 秒**），慢的是刷新；
- `/api/transcript` 曾对**每个任务**各取一遍该运行的全部事件：历史是同一份日志的纯函数，5 个任务就是 5 倍白解析；
- 而事件里 **98.8% 的字节**是 `ModelReturned` 携带的上游逐条消息 usage 归属（单条可达 388 KB），历史派生**一行都不读**——为 0.2% 的正文解析 99% 的负载。

现在：`store.events(types=)` 按类型过滤、`task_history(task, events)` 由调用方共用一份事件、回放走 `event_index()` 只取三列、任务详情走 `events_task` 索引取尾部 200 条、`/api/state` 剔掉了界面从不读的文件台账字段。同一个 run 上：`/api/transcript` **5 655 ms → 38 ms**，`/api/task` **1 273 ms → 31 ms**，`/api/state` **1 038 KB → 244 KB**。

**仍然存在的边界**：库体积的 93% 是上游 usage 归属（事件表 158 MB / 169 MB），运行时不再解析它，但仍在如实记录——是否只留聚合值是一个"记录什么事实"的决定，见实施记录 10.5。
