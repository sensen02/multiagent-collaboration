...
- **长目标会自己让位**（对话页）：主目标长的时候它一个人就能占满整个对话视口——实测某个 1060 字的目标渲染成 524px，而对话区视口只有 435px，于是整屏只有目标、一句对话都看不到。现在**超过 3 行**的目标在下滑时自动折叠成 2 行（带「展开目标」按钮），滑回顶部自动展开；短目标完全不进这个机制，也不会多出按钮。折叠会按被收起的高度补偿滚动位置，正在看的内容不会跳。
